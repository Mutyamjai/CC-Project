...
import { fetch_ready_to_collect_orders } from '../../../Services/Service_Functions/laundry'
import { useSelector } from 'react-redux';
import Spinner from '../../../Components/Common/Spinner';
import { useState } from 'react';
import { useForm } from 'react-hook-form';
import ReadyToCollectCard from './ReadyToCollectCard';
    ...
    return (
        <div className="bg-black bg-opacity-90 min-h-screen p-8 overflow-x-hidden w-full">
            ...
            <form onSubmit={handleSubmit(on_submit)}>
                ...
                className="bg-gray-800 text-white py-4 px-6 border border-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"></input>
            ...
            <h1 className='text-blue-300 font-bold text-center mb-3 text-2xl'>SEARCHED ORDER</h1>
            {
                searched_order && (
                    <div className="mb-8 flex justify-center">
                        <ReadyToCollectCard data={searched_order} set_confirmation_model={set_confirmation_model}
            ...
            <h1 className="text-blue-300 font-bold text-center mb-5 text-2xl"> READY TO COLLECT ITEMS</h1>
            <div className={`grid ${details.length > 1 ? 'grid-cols-1 md:grid-cols-2' : 'flex justify-center'}`}>
            {
                details.map((item, index) => (
                    <ReadyToCollectCard key={index} data={item} set_confirmation_model={set_confirmation_model}
                            set_loading={set_loading}/>
                ))
            }
            </div>
            <div className='flex justify-center'>
                    {
                        details.length === 0 && (
                            <p className=' text-white text-xl font-bold'>NO ORDERS ARE THERE TO COLLECT !!</p>
                        )
                    }
            </div>
